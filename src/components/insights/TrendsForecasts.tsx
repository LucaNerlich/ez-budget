...
export default function TrendsForecasts() {
    ...
    const monthly = useMemo(() => {
        const rows: Array<{ key: string; year: number; month: number; income: number; expense: number; net: number }> = [];
        ...
        for (let i = 0; i < years.length; i++) {
            ...
            for (let j = 0; j < months.length; j++) {
                ...
                rows.push({ key: `${y}-${m < 10 ? '0'+m : m}`, year: y, month: m, income, expense, net: income + expense });
            }
        }
    ...
    const forecastConfig = useMemo(() => {
        ...
        return {
            ...
            datasets: [
                ...
                { label: 'Forecast (6M)', data: [...Array(net.length).fill(null), ...forecast], borderColor: '#999', borderDash: [6, 6], backgroundColor: 'transparent' }
            ]
        };
    }, [monthly]);
        ...
        <div className="mt-4">
            ...
            <div className="row g-4">
                <div className="col-12">
                    ...
                    <Chart type="line" data={rollingConfig} options={{ responsive: true, plugins: { legend: { position: 'bottom' } } }} />
                </div>
                <div className="col-12 col-md-6">
                    <h3>Year‑over‑Year (Netto)</h3>
                    <Chart type="bar" data={yoyConfig} options={{ responsive: true, plugins: { legend: { display: false } }, aspectRatio: 1 }} />
                </div>
                <div className="col-12 col-md-6">
                    <h3>Forecast (6 Monate)</h3>
                    <Chart type="line" data={forecastConfig} options={{ responsive: true, plugins: { legend: { position: 'bottom' } }, aspectRatio: 1 }} />
                </div>
            </div>
        </div>
    );
